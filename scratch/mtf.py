...
def calculate_mtf_stretch_parameters_for_channel(stretch_params, channel):
    ...
    indx_clip = (channel < 1.0) & (channel > 0.0)
...
def stretch_channel(channel, mtf_stretch_params):
    ...
    new_channel[new_channel <= mtf_stretch_params.shadow_clipping] = 0.0
    new_channel[new_channel >= mtf_stretch_params.highlight_clipping] = 1.0

    indx_inside = (new_channel > mtf_stretch_params.shadow_clipping) & (
        new_channel < mtf_stretch_params.highlight_clipping
    )

    new_channel[indx_inside] = (
        new_channel[indx_inside] - mtf_stretch_params.shadow_clipping
    ) / (mtf_stretch_params.highlight_clipping - mtf_stretch_params.shadow_clipping)

    return MTF(new_channel, mtf_stretch_params.midtone)
...
if __name__ == "__main__":
    ...
    # data = np.random.rand(3008,3008)
    fp = astropy.io.fits.open("pleiades.fits")
    fp.info()
    data = fp[0].data
    data = data / (2**16 - 1)
    ...
    stretch_params = StretchParameters("10% Bg, 3 sigma")
    t0 = time.time()
    mtf_params = calculate_mtf_stretch_parameters_for_channel(stretch_params, data)
    stretched = stretch_channel(data, mtf_params)
