...
from chimera.interfaces.fan import FanStatus, FanControllabeSpeed, FanControllabeDirection, FanState
...
from chimera.core.exceptions import ObjectTooLowException, ObjectNotFoundException, printException
from chimera.interfaces.telescope import SlewRate, TelescopeStatus, TelescopePierSide, TelescopePier, TelescopeCover
import sys
import copy
...
class ChimeraTel (ChimeraCLI):
    ...
    def __init__(self):
        ChimeraCLI.__init__(
            self, "chimera-tel", "Telescope controller", 0.1)
        ...
        self.addParameters(dict(name="fan", default="/Fan/0", helpGroup="FANS", help="Fan instrument to be used."))
    ...
    @action(name="open_cover", help="Open telescope cover", long="open-cover", helpGroup="COVER", actionGroup="COVER")
    ...
    @action(name="close_cover", help="Close telescope cover", long="close-cover", helpGroup="COVER", actionGroup="COVER")
    ...
    @action(long="fan-speed", type="float", help="Set fan speed.", metavar="speed", helpGroup="FANS",
            actionGroup="FANS")
    ...
    @action(long="fan-on", help="Start telescope fan", helpGroup="FANS", actionGroup="FANS")
    ...
    @action(long="fan-off", help="Stop telescope fan", helpGroup="FANS", actionGroup="FANS")
    ...
    def info(self, options):
        ...
        if self.telescope.features(TelescopeCover):
            self.out("telescope cover: %s " % ('open' if telescope.isCoverOpen() else 'closed'))
        if self.telescope.features(TelescopePier):
            self.out("current side of pier: %s " % telescope.getPierSide().__str__().lower())
        ...
        if self.telescope['fans'] is not None:
            for fan in self.telescope['fans']:
                ...
                rotation = ' speed %.2f' % fan.getRotation() if fan.features(FanControllabeSpeed) else ''
                direction = ' direction %s' % fan.getDirection() if fan.features(FanControllabeDirection) else ''
                self.out("Fan %s: %s%s%s" % (fan.getLocation(), stats, rotation, direction))
    ...
    @action(help="Move telescope to EAST pierside", long="side-east", helpGroup="PIERSIDE", actionGroup="PIERSIDE")
    ...
    @action(help="Move telescope to WEST pierside", long="side-west", helpGroup="PIERSIDE", actionGroup="PIERSIDE")
