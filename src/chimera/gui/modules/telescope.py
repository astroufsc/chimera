...
class TelescopeView:
    ...
    def __init__(self, module):
        ...
        self.objectNameCombo = self.module.builder.get_object(
            "objectNameCombo")
    ...
    def updateStatusView(self):
        ...
        if not self.module.controller.isSlewing():
            ...
            self.raLabel.set_markup(
                "<span font-family='monospace' size='14000'>%10s<span size='9000'>%4s</span></span>" % (raInt, raFrac))
            self.decLabel.set_markup(
                "<span font-family='monospace' size='14000'>%10s<span size='9000'>%4s</span></span>" % (decInt, decFrac))
            self.altLabel.set_markup(
                "<span font-family='monospace' size='14000'>%10s<span size='9000'>%4s</span></span>" % (altInt, altFrac))
            self.azLabel.set_markup(
                "<span font-family='monospace' size='14000'>%10s<span size='9000'>%4s</span></span>" % (azInt, azFrac))
            self.lstLabel.set_markup(
                "<span font-family='monospace' size='14000'>%10s<span size='9000'>%4s</span></span>" % (lstInt, lstFrac))
            self.localtimeLabel.set_markup(
                "<span font-family='monospace' size='14000'>%10s<span size='9000'>.000</span></span>" % localtimeInt)
    ...
    def startStatusUpdate(self):
        self.status_timeout = glib.timeout_add(
            1500, self.updateStatusView, priority=glib.PRIORITY_LOW)
    ...
    def updateTrackingStatus(self):
        ...
        if(self.module.controller.isTracking()):
            ...
            self.module.builder.get_object(
                "trackingCheckbox").set_active(False)
    ...
    def slewBeginUi(self):
        def ui():
            ...
            telescopeProgress = self.module.builder.get_object(
                "telescopeProgress")
...
class TelescopeGUIModule(ChimeraGUIModule):
    ...
    def setupGUI(self, objects):
        ...
        self.builder.add_from_file(
            os.path.join(os.path.dirname(__file__), "telescope.xml"))
