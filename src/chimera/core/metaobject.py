...
class MetaObject (type):
    ...
    def __new__(meta, clsname, bases, _dict):
        ...
        for name, obj in _dict.items():
            ...
            if hasattr(obj, '__call__') and not name.startswith('_'):
                ...
                if hasattr(obj, EVENT_ATTRIBUTE_NAME):
                    _dict[name] = MethodWrapper(
                        obj, dispatcher=EventWrapperDispatcher)
                    events.append(name)

                # auto-locked methods
                elif hasattr(obj, LOCK_ATTRIBUTE_NAME):
                    _dict[name] = LockWrapper(
                        obj, dispatcher=LockWrapperDispatcher)
                    methods.append(name)

                # normal objects
                else:
                    _dict[name] = MethodWrapper(
                        obj, dispatcher=MethodWrapperDispatcher)
                    methods.append(name)
        ...
        _dict[INSTANCE_MONITOR_ATTRIBUTE_NAME] = threading.Condition(
            threading.RLock())
