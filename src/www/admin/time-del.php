...
$res =& $db->query("DELETE FROM user_sched WHERE id = " . $_GET['id']);

if (DB::isError($res)) {

        Header("Location: " . getError("user.php?id=" . $_GET['user_id'], "Houce um erro ao tentar remover o per&iacute;odo. Tente novamente."));
        exit(1);

} else {

        Header("Location: " . getMsg("user.php?id=" . $_GET['user_id'], "O per&iacute;odo foi removido com sucesso."));
        exit(0);
