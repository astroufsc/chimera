...
from chimera.interfaces.fan import FanControllabeSpeed, FanControllabeDirection, FanStatus, FanState
...
class ChimeraDome(ChimeraCLI):
    def __init__(self):
        ChimeraCLI.__init__(
            self, "chimera-dome", "Dome controller", 0.1)
        ...
        self.addParameters(dict(name="fan",
                                default="/Fan/0",
                                helpGroup="DOMEFANS",
        ...
        self.addParameters(dict(name="lamp",
                                default="/Lamp/0",
                                helpGroup="LIGHT",
    ...
    @action(long="open-slit", help="Open dome slit", helpGroup="SHUTTER", actionGroup="SLIT")
    def openSlit(self, options):
        self.out("Opening dome slit ... ", end="")
        self.dome.openSlit()
        self.out("OK")

    @action(long="close-slit", help="Close dome slit", helpGroup="SHUTTER", actionGroup="SLIT")
    def closeSlit(self, options):
        self.out("Closing dome slit ... ", end="")
        self.dome.closeSlit()
        self.out("OK")

    @action(long="open-flap", help="Open dome flap", helpGroup="SHUTTER", actionGroup="FLAP")
    def openFlap(self, options):
        self.out("Opening dome flap ... ", end="")
        self.dome.openFlap()
        self.out("OK")

    @action(long="close-flap", help="Close dome flap", helpGroup="SHUTTER", actionGroup="FLAP")
    def closeFlap(self, options):
        self.out("Closing dome flap ... ", end="")
        self.dome.closeFlap()
        self.out("OK")

    @action(long="light-off", help="Turn light off", helpGroup="LIGHT", actionGroup="LIGHT")
    ...
    @action(long="light-on", help="Turn light on", helpGroup="LIGHT", actionGroup="LIGHT")
    ...
    @action(long="light-intensity", type="float", metavar="intensity", help="Set light intensity to specified value.",
            helpGroup="LIGHT", actionGroup="LIGHT")
    ...
    @action(help="Stop tracking the telescope (stand)", helpGroup="TELESCOPE", actionGroup="TRACKING")
    def stand(self, options):
        self.out("Deactivating tracking ... ", end="")
        self.dome.stand()
        self.out("OK")

    @action(long="to", type="string", help="Move dome to AZ azimuth", metavar="AZ", helpGroup="COMMANDS")
    ...
    @action(long="fan-speed", type="float", help="Set fan speed.", metavar="speed", helpGroup="DOMEFANS",
    ...
    @action(long="fan-on", help="Start dome fan", helpGroup="DOMEFANS", actionGroup="DOMEFANS")
    ...
    @action(long="fan-off", help="Stop dome fan", helpGroup="DOMEFANS", actionGroup="DOMEFANS")
    ...
    def info(self, options):
        ...
        if self.dome['lamps'] is not None:
            for lamp in self.dome['lamps']:
                ...
                dimming = ' intensity: %3.2f' % lamp.getIntensity() if lamp.features(LampDimmer) else ''
        ...
        if self.dome['fans'] is not None:
            for fan in self.dome['fans']:
                ...
                rotation = ' speed %.2f' % domefan.getRotation() if domefan.features(FanControllabeSpeed) else ''
                direction = ' direction %s' % domefan.getDirection() if domefan.features(FanControllabeDirection) else ''
                self.out("Fan %s: %s%s%s" % (domefan.getLocation(), stats, rotation, direction))
