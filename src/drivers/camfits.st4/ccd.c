...
int main()
{
	...
	puts("ST-4 test program.");
	printf("tty = %s\n\n", deftty);
	
	camera = alloc_camera(deftty);
	if (camera == NULL) {
		puts("alloc_camera()");
		exit(1);
	}
...
/* initializes camera's comm parameters */
...
/* allocates memory for camera's structures */
...
/* frees the memory allocated for camera's structures */
...
/* sets up all the camera's parameters */
